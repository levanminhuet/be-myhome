...
export const reportPost = ({ pid, reason }) => new Promise(async (resolve, reject) => {
    try {
...
export const getReports = ({ page, limit, order, ...query }) => new Promise(async (resolve, reject) => {
    try {
        const queries = {}
        const step = !page ? 0 : +page - 1
        const lim = +limit || +process.env.LIMIT_ADMIN
...
export const updateReport = ({ rid, status, pid }) => new Promise(async (resolve, reject) => {
    try {
        ...
        resolve({
            err: response > 0 ? 0 : 1,
            data: response > 0 ? 'Đã xóa bài đăng vi phạm' : 'Something went wrong',
        })
    } catch (error) {
        reject(error)
    }
